...
4、元素的单选和多选
4、编辑区域移动单个或多个组件，多选的时候编辑区域根据组件之间不同的位置，生成一个红框把选中的组件进行框选
5、画布的缩放，一旦画布缩放，组件的大小、拖拽和拖放的距离也需要跟着调整
6、画布缩放的时候使用 transform:scale(zoom / 100)，需要搭配 transformOrigin(50%,0) 使用，不然放大的时候会从中间开始覆盖 header 部分
7、编辑区域拓展为伸缩框，可以对当前编辑区域的组件的大小进行缩放，伸缩的时候，组件的宽高不能小于 2px
